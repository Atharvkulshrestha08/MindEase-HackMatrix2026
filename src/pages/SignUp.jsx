...
const SignUp = () => {
  ...
  const onSubmit = async (data) => {
    ...
    setError(null)
    try {
      const { error } = await signUp(data.email, data.password, { full_name: data.name })
      if (error) {
    ...
    <div className="min-h-screen flex">
      ...
      >
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/3 right-1/4 w-72 h-72 rounded-full" style={{ backgroundColor: 'rgba(255,255,255,0.1)', filter: 'blur(60px)' }} />
          <div className="absolute bottom-1/3 left-1/4 w-96 h-96 rounded-full" style={{ backgroundColor: 'rgba(255,255,255,0.05)', filter: 'blur(80px)' }} />
        </div>
        <div className="relative z-10 text-center text-white p-16">
          ...
          >
            ...
            <p className="text-xl max-w-sm" style={{ opacity: 0.85 }}>
              Start your wellness journey today. Small steps, big changes.
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 md:p-12">
        ...
        >
          ...
          <p className="text-lg mb-10" style={{ color: 'var(--on-surface-variant)' }}>
            A calm space to track your mood and build healthier habits
          </p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
            ...
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-[42px]" style={{ color: 'var(--on-surface-variant)' }}>
            ...
            {error && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-[14px] text-sm" style={{ backgroundColor: 'color-mix(in srgb, var(--error) 10%, transparent)', color: 'var(--error)' }}>
                {error}
              </motion.div>
            )}
          ...
          <Button variant="outline" fullWidth size="lg" onClick={handleGoogleSignUp} disabled={loading}>
