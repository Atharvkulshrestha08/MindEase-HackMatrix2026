...
/**
 * Animated purchase/CTA button with slide-in color effect.
 * Based on Uiverse.io design by Creatlydev.
 * 
 * @param {string} color - The accent color for the button (e.g. '#006491')
 * @param {string} text - Button label text
 * @param {React.ReactNode} icon - Optional custom icon (defaults to send/arrow SVG)
 * @param {function} onClick - Click handler
 * @param {string} className - Additional classes
 */
const PurchaseButton = ({ color = '#006491', text = 'Get Started', icon, onClick, className, style, ...props }) => {
  return (
    <button
      className={clsx('purchase-btn', className)}
      style={{ '--clr': color, ...style }}
      onClick={onClick}
      {...props}
    >
      <span className="purchase-btn-decor" />
      <div className="purchase-btn-content">
        <div className="purchase-btn__icon">
          {icon || (
            <svg viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg" width="24">
              <circle opacity="0.5" cx="25" cy="25" r="23" fill="url(#purchase-btn-gradient)" />
              <mask id="purchase-btn-mask" fill="#fff">
        ...
        <span className="purchase-btn__text">{text}</span>
      </div>
    </button>
  )
}
